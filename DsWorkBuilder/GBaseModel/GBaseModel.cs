...
namespace Builder
{
	public class GBaseModel:BaseModel, IDbModel
	{
		...
		private List<ColumnModel> GetColumn(string dbName, string tableName, List<string> pkList)
		{
			...
			foreach(DataRow dr in dt.Rows)
			{
				string name = dr["COLUMN_NAME"].ToString().ToUpper();
		...
		private DataTable GetData(string sql)
		{
			...
			//return ds.Tables[0];
			GBaseDataAdapter adapter = new GBaseDataAdapter(sql, this.ConnectUrl);
		...
		{
			string database = "";
			GBaseConnection connection = new GBaseConnection();
